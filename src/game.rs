...
impl Game {
    ...
    fn add_food(&mut self) {
        ...
        let mut next_x = rng.gen_range(1..self.width - 1);
        let mut next_y = rng.gen_range(1..self.height - 1);

        while self.snake.overlap_tail(next_x, next_y) {
            next_x = rng.gen_range(1..self.width - 1);
            next_y = rng.gen_range(1..self.height - 1);
        }
